...
def do(content,mode):
	...
	if mode == 'code_obf':
		data = content[0]
		lang = content[1]
		encode_type = content[2]
		lang_counter = content[3]
		lang_encode_counter = content[4]
		input_file_counter = content[5]
		if lang == 'javascript':
				if encode_type == 'simple_hex':
					from lib.encoder.javascript import simple_hex
					data = simple_hex.start(data)
		done.encode_res(sys.argv[lang_counter],sys.argv[lang_encode_counter],sys.argv[input_file_counter])
		return data